...
const service = axios.create({
    baseURL: 'http://ggkt2flfo.v5.idcfengye.com', // api 的 base_url
...
service.interceptors.response.use(response => {
        if (response.data.code == 208) {
            //debugger
            // 替换# 后台获取不到#后面的参数
            let url = window.location.href.replace('#', 'guiguketang')
            window.location = 'http://ggkt2flfo.v5.idcfengye.com/api/user/wechat/authorize?returnUrl=' + url
        } else {
