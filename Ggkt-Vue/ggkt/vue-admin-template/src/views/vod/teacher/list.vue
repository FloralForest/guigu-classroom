...
export default {
  ...
  methods:{ //具体方法
    ...
    batchRemove() {
      ...
      this.$confirm('此操作将删除所选讲师信息, 是否继续?', '提示', {
    ...
    //跳转到添加表单页面($router路由跳转)
    add() {
      this.$router.push({path:'/vod/teacher/create'}) //路由的文件中可找到该路径(router/index.js)
    },
    fetchData() {
      //ajax
      teacherApi.pageList(this.page,this.limit,this.searchObj)
        .then(response => {
          this.list = response.data.records //数据
